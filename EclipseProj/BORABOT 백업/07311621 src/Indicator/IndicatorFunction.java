...
public class IndicatorFunction {
	...
	public static double[][] get_HLCV_HistoryArray(CryptowatchAPI crypt, String exchange, String coin, String base, int interval, int period_day) throws Exception {
		...
		double[][] historyArray = new double[size][4];
	...
	public static double[] getHistoryMeanPriceArray_bt(CryptowatchAPI crypt, String exchange, String coin, String base, int interval, long after) throws Exception {

		Gson gson = new Gson();
		Date date = new Date();
		
		String ohlc_string = crypt.getOHLC(exchange, coin, base, after, interval);
		JsonObject ohlc_json = new JsonParser().parse(ohlc_string).getAsJsonObject();
		String ohlc_result_string = gson.toJson(ohlc_json.get("result"));
		JsonObject ohlc_result_json = new JsonParser().parse(ohlc_result_string).getAsJsonObject();
		JsonArray ohlc_jsarr = ohlc_result_json.get(Long.toString(interval)).getAsJsonArray();
